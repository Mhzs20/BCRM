...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-8">
        <h1 class="text-3xl font-extrabold text-gray-900 text-center">ارسال پیامک گروهی</h1>
        <a href="{{ route('admin.bulk-sms.history') }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
            <i class="ri-history-line ml-2"></i> تاریخچه ارسال
        </a>
    </div>
    ...
    <div class="bg-white shadow-xl rounded-lg p-6 mb-8">
        <form action="{{ route('admin.bulk-sms.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 items-end">
            ...
            <div class="flex flex-col sm:flex-row space-y-2 w-full sm:space-y-0 sm:space-x-4 col-span-full justify-between mt-4">
                ...
                <a href="{{ route('admin.bulk-sms.index') }}" class="inline-flex items-center px-5 py-2.5 border border-gray-300 text-sm font-medium rounded-md shadow-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition ease-in-out duration-150">
    ...
    <div class="bg-white shadow-xl rounded-lg overflow-hidden mt-8">
        <form action="{{ route('admin.bulk-sms.send') }}" method="POST">
            @csrf
            ...
            <div class="p-6">
                <div class="mb-6">
                    ...
                    <textarea name="message" id="message" rows="4" required class="form-textarea w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 p-2.5 text-gray-900 bg-gray-50 hover:bg-gray-100 transition duration-150 ease-in-out" placeholder="متن پیامکی که می‌خواهید ارسال کنید را وارد کنید..."></textarea>
                    <div class="text-sm text-gray-500 mt-1">
                        <span id="char-count">0</span> کاراکتر / حداکثر 500 کاراکتر
                    </div>
                ...
                <div class="mb-6">
                    <label class="flex items-center">
                        <input type="checkbox" id="send-to-all" class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                        <span class="mr-2 text-sm text-gray-700">ارسال به همه سالن‌های فیلتر شده</span>
                    </label>
                </div>
                ...
                <div class="mt-4 text-sm text-gray-600">
                    <p><strong>تعداد کل سالن‌های فیلتر شده:</strong> {{ $salons->total() }}</p>
                    <p id="selected-count"><strong>تعداد انتخاب شده:</strong> 0</p>
                </div>
            </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($salons as $salon)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                @if(isset($salon->lastSmsPurchaseDate) && $salon->lastSmsPurchaseDate)
                                    {{ \Morilog\Jalali\Jalalian::fromCarbon($salon->lastSmsPurchaseDate)->format('Y/m/d') }}
                                @else
                                    تاکنون خرید نکرده
                                @endif
                            </td>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        async function fetchAndPopulateCities(provinceId, selectedCityId = null) {
            citySelect.innerHTML = '<option value="">همه</option>';
            if (!provinceId) {
                return;
            }
            try {
        ...
        async function fetchAndPopulateSubcategories(categoryId, selectedSubcategoryId = null) {
            businessSubcategorySelect.innerHTML = '<option value="">همه</option>';
            if (!categoryId) {
                return;
            }
            try {
