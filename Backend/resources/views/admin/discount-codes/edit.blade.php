...
@section('content')
        ...
        <div class="bg-white shadow-lg rounded-xl overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-indigo-500 to-purple-600 px-8 py-6">
                <div class="flex items-center">
                    ...
                    <div>
                        <h3 class="text-xl font-bold text-white">ویرایش کد تخفیف</h3>
                        <p class="text-indigo-100 text-sm">کد تخفیف خود را با تنظیمات دلخواه ویرایش کنید</p>
                    </div>
                </div>
            </div>
            ...
            <form action="{{ route('admin.discount-codes.update', $discountCode) }}" method="POST">
                ...
                <div class="space-y-8">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="text" 
                                           name="code" 
                                           id="code"
                                           value="{{ old('code', request('form_code', $discountCode->code)) }}"
                                           required
                            ...
                            <div>
                                ...
                                <textarea name="description" 
                                          ...
                                          class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200 @error('description') border-red-500 @enderror">{{ old('description', request('form_description', $discountCode->description)) }}</textarea>
                                @error('description')
                                    <p class="mt-2 text-sm text-red-600 flex items-center">
                                        <i class="ri-error-warning-line ml-1"></i>
                                        {{ $message }}
                    ...
                    <div class="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-6 border border-green-100">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select name="type" id="type" required class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200 @error('type') border-red-500 @enderror">
                                    <option value="percentage" {{ old('type', request('form_type', $discountCode->type)) === 'percentage' ? 'selected' : '' }}>
                                        <i class="ri-percent-line"></i> درصدی
                                    </option>
                                    <option value="fixed" {{ old('type', request('form_type', $discountCode->type)) === 'fixed' ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="number" 
                                           name="value" 
                                           id="value"
                                           value="{{ old('value', request('form_value', $discountCode->value)) }}"
                                           required
                                           min="0"
                                           step="0.01"
                                           placeholder="0"
                        ...
                        <div class="mt-6">
                            ...
                            <input type="number" 
                                   ...
                                   id="min_order_amount"
                                   value="{{ old('min_order_amount', request('form_min_order_amount', $discountCode->min_order_amount)) }}"
                                   min="0"
                                   step="1000"
                    ...
                    <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl p-6 border border-purple-100">
                        ...
                        <div>
                            ...
                            <input type="number" 
                                   name="usage_limit" 
                                   id="usage_limit"
                                   value="{{ old('usage_limit', request('form_usage_limit', $discountCode->usage_limit)) }}"
                                   min="1"
                    ...
                    <div class="bg-gradient-to-br from-orange-50 to-yellow-50 rounded-xl p-6 border border-orange-100">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <label class="relative cursor-pointer radio-option">
                                    <input type="radio" name="user_filter_type" value="all" 
                                           {{ old('user_filter_type', request('form_user_filter_type', $discountCode->user_filter_type ?? 'all')) === 'all' ? 'checked' : '' }}
                                ...
                                <label class="relative cursor-pointer radio-option">
                                    <input type="radio" name="user_filter_type" value="filtered" 
                                           {{ old('user_filter_type', request('form_user_filter_type', $discountCode->user_filter_type)) === 'filtered' || request('filter_applied') ? 'checked' : '' }}
                                           class="sr-only radio-input">
                                    <div class="radio-card flex items-center p-4 border-2 border-gray-200 rounded-lg transition-all duration-200 hover:border-gray-300">
                                        <div class="radio-circle flex items-center justify-center w-6 h-6 border-2 border-gray-300 rounded-full mr-3 transition-all duration-200">
                                            <div class="radio-dot w-3 h-3 bg-indigo-500 rounded-full transform scale-0 transition-transform duration-200"></div>
                        ...
                        <div class="hidden">
                            <input type="hidden" name="filter_province_id" value="{{ request('province_id', $discountCode->target_users['province_id'] ?? '') }}">
                            <input type="hidden" name="filter_city_id" value="{{ request('city_id', $discountCode->target_users['city_id'] ?? '') }}">
                            <input type="hidden" name="filter_business_category_id" value="{{ request('business_category_id', $discountCode->target_users['business_category_id'] ?? '') }}">
                    ...
                    <div class="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-lg shadow-sm">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" 
                                   ...
                                   id="is_active"
                                   value="1"
                                   {{ old('is_active', request('form_is_active', $discountCode->is_active)) == '1' || old('is_active', request('form_is_active', $discountCode->is_active)) === 1 ? 'checked' : '' }}
                                   class="sr-only peer">
    ...
    <div id="filter-options" class="mt-8 bg-white shadow-lg rounded-xl overflow-hidden" style="display: {{ request('filter_applied') || old('user_filter_type', request('form_user_filter_type', $discountCode->user_filter_type)) === 'filtered' ? 'block' : 'none' }};">
        <!-- Filter Header -->
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                <div>
                    ...
                    <select id="filter_province_id" class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200">
                        ...
                        @foreach($provinces as $province)
                            <option value="{{ $province->id }}" {{ request('province_id', $discountCode->target_users['province_id'] ?? '') == $province->id ? 'selected' : '' }}>
                                {{ $province->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select id="filter_city_id" class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200">
                        ...
                        @if(isset($cities))
                            @foreach($cities as $city)
                                <option value="{{ $city->id }}" {{ request('city_id', $discountCode->target_users['city_id'] ?? '') == $city->id ? 'selected' : '' }}>
                                    {{ $city->name }}
                                </option>
                            @endforeach
                        @endif
                ...
                <div>
                    ...
                    <select id="filter_business_category_id" class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200">
                        ...
                        @foreach($businessCategories as $category)
                            <option value="{{ $category->id }}" {{ request('business_category_id', $discountCode->target_users['business_category_id'] ?? '') == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select id="filter_business_subcategory_id" class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200">
                        ...
                        @if(isset($businessSubcategories))
                            @foreach($businessSubcategories as $subcategory)
                                <option value="{{ $subcategory->id }}" {{ request('business_subcategory_id', $discountCode->target_users['business_subcategory_id'] ?? '') == $subcategory->id ? 'selected' : '' }}>
                                    {{ $subcategory->name }}
                                </option>
                            @endforeach
                        @endif
                ...
                <div>
                    ...
                    <select id="filter_status" class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200">
                        ...
                        <option value="1" {{ request('status', $discountCode->target_users['status'] ?? '') === '1' ? 'selected' : '' }}>فعال</option>
                        <option value="0" {{ request('status', $discountCode->target_users['status'] ?? '') === '0' ? 'selected' : '' }}>غیرفعال</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="filter_last_sms_purchase" class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200">
                        <option value="">همه</option>
                        <option value="last_month" {{ request('last_sms_purchase', $discountCode->target_users['last_sms_purchase'] ?? '') === 'last_month' ? 'selected' : '' }}>یک ماه اخیر</option>
                        <option value="last_3_months" {{ request('last_sms_purchase', $discountCode->target_users['last_sms_purchase'] ?? '') === 'last_3_months' ? 'selected' : '' }}>سه ماه اخیر</option>
                        <option value="last_6_months" {{ request('last_sms_purchase', $discountCode->target_users['last_sms_purchase'] ?? '') === 'last_6_months' ? 'selected' : '' }}>شش ماه اخیر</option>
                        <option value="more_than_6_months" {{ request('last_sms_purchase', $discountCode->target_users['last_sms_purchase'] ?? '') === 'more_than_6_months' ? 'selected' : '' }}>بیشتر از شش ماه</option>
                        <option value="never" {{ request('last_sms_purchase', $discountCode->target_users['last_sms_purchase'] ?? '') === 'never' ? 'selected' : '' }}>تاکنون خرید نکرده</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="filter_monthly_sms_consumption" class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200">
                        <option value="">همه</option>
                        <option value="high" {{ request('monthly_sms_consumption', $discountCode->target_users['monthly_sms_consumption'] ?? '') === 'high' ? 'selected' : '' }}>زیاد (بیشتر از ۵۰۰)</option>
                        <option value="medium" {{ request('monthly_sms_consumption', $discountCode->target_users['monthly_sms_consumption'] ?? '') === 'medium' ? 'selected' : '' }}>متوسط (۱۰۰ تا ۵۰۰)</option>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (applyFilterBtn) {
            applyFilterBtn.addEventListener('click', function() {
                ...
                window.location.href = '{{ route("admin.discount-codes.edit", $discountCode) }}?' + params.toString();
            });
        }
