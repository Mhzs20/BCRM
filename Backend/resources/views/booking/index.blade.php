                ...
                <div id="calendar-section" class="hidden mt-6">
                    ...
                    <div class="self-stretch flex flex-col justify-start items-start gap-3 sm:gap-5" dir="rtl">
                        ...
                        <div id="calendar-days" class="self-stretch flex flex-col justify-start items-start gap-1 sm:gap-1.5">
                            <!-- Calendar Loader -->
                            <div id="calendar-loader" class="w-full flex flex-col items-center justify-center py-12 gap-4">
                                <div class="animate-spin rounded-full h-12 w-12 border-4 border-gray-200 border-t-orange-500"></div>
                                <p class="text-neutral-500 text-sm font-iranyekan">در حال بارگذاری تقویم...</p>
                            </div>
                        </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const allowHolidayBooking = {{ json_encode($salon->online_booking_settings['allow_holiday_booking'] ?? false) }};
    ...
    function generateCalendar() {
        ...
        for (let w = 0; w < weeks; w++) {
            ...
            weekDiv.className = 'w-full self-stretch h-14 sm:h-16 inline-flex justify-center items-center gap-1 sm:gap-1.5 md:gap-2';
                ...
                // Allow click if not past date, day is enabled, and either holidays are allowed or it's not a holiday
                if (!isPastDate && isDayEnabled && (allowHolidayBooking || !isHolidayFlag)) {
                    cell.addEventListener('click', function() { selectCalendarDate(this); });
                } else if (!isPastDate && isDayEnabled && isHolidayFlag && !allowHolidayBooking) {
                    // Add disabled class for holidays when not allowed
